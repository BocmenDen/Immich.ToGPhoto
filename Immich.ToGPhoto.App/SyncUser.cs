...
namespace Immich.ToGPhoto.App
{
    public class SyncUser
    {
        ...
        public async Task SyncNewPhotos(ILogger logger)
        {
            ...
            using var db = GetDB();
            await LoadPhotos(db, logger);
            await DeletePhotos(db, logger);
            await DeleteAnotherPhotos(db, logger);
        }

        /// <summary>
        /// Удаление фото которые не были загружены в Immich, но есть в Google Photos
        /// </summary>
        private async Task DeleteAnotherPhotos(SyncDB db, ILogger logger)
        {
            List<string> toDeleate = [];
            await foreach (var chank in _gpmcClient.GetKeys().Chunk(CHANK_SIZE))
            {
                var findElems = await db.SyncItems.Where(x => chank.Contains(x.GoogleKey)).ToListAsync();
        ...
        /// </summary>
        private async Task DeletePhotos(SyncDB db, ILogger logger)
        {
            ...
            {
                var syncItems = await db.SyncItems.Where(x => itemImmichChank.Contains(x.ImmichKey)).ToListAsync();
                ...
                db.SyncItems.RemoveRange(syncItemsToRemove);
        ...
        /// </summary>
        private async Task LoadPhotos(SyncDB db, ILogger logger)
        {
            ...
            await foreach (var itemImmichChank in allAssetsImmich.Chunk(CHANK_SIZE))
            {
                ...
                var keysDBUploaded = await db.SyncItems.Where(x => itemImmichChank.Select(x => x.Id).Contains(x.ImmichKey)).ToListAsync();
                ...
                if (toResetUpload.Any())
                {
                    ...
                    db.SyncItems.RemoveRange(toResetUpload);
                    await db.SaveChangesAsync();
                }
        ...
        private async Task CheckConstraintConfict(SyncDB db, IDictionary<Uri, string> resultUpload)
        {
            await using var conflictItems = GetConflictItems(db, resultUpload).GetAsyncEnumerator();
        ...
        private async IAsyncEnumerable<(string fileName, string googleKey)> GetConflictItems(SyncDB db, IDictionary<Uri, string> resultUpload)
        {
            await foreach (var item in db.SyncItems.Where(x => resultUpload.Values.Contains(x.GoogleKey)).AsAsyncEnumerable())
            {
